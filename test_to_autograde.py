...
def parse_unittests(tests):
    ...
    test_list = []
    for test in tests:
        test_list.append({"description": test.shortDescription(), "command": f"python3 -m unittest {test.id()}"})
...
def parse_pytests(tests):
    ...
    for test in tests:
        ...
        test_list.append({"description": test_func, "command": f"pytest{test_file}::{test_func}"})
